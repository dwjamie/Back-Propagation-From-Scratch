...
class BackProp:
    def __init__(self, update_rule, hidden_layer_size=5, learning_rate=0.1, max_iter=1000):
...
if __name__ == '__main__':
    ...
    X = pd.get_dummies(data.iloc[:, 1:-1], drop_first=True).values
    y = pd.get_dummies(data.好瓜).values

    update_rules = ('standard', 'cumulative')
    for update_rule in update_rules:
        classifier = BackProp(update_rule)
        classifier.fit(X, y)
        score = classifier.score(X, y)
        print('Accuracy (' + update_rule + ' BP algorithm):', score)
